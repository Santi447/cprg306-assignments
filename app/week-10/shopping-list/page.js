...
export default function Page(){
  ...
  if (!user){
    return(
      <div className="bg-slate-950 min-h-screen flex flex-col items-center justify-center">
        ...
        <Link href="/week-10/login" className="text-blue-600 hover:text-blue-300">
          Back to Login
        </Link>
      </div>
    )
  ...
  return(
    ...
    <div className="flex justify-center items-center p-4">
      {selectedItem && (
        <DeleteButton userId={user.uid} item={selectedItem} />
      )}
    </div>
    <div className="flex flex-row justify-center">
      <div className = "flex flex-col items-center">
      <NewItem onAddItem={handleAddItem} />
      <ItemList onItemSelect={handleItemSelected} />
    </div>
    {selectedItemName && (
      <div>
